...
class EditSalesQuotationState extends State<EditSalesQuotationScreen> {
          ...
          ApiAdmin().getQuotationById(widget.id).then((value) => {
                setState(() {
                  ...
                  if (response['status'] == '200') {
                    ...
                    _additem.text = _item[0];
                    _additemrate.text = _amount[0];
                    _partymobile.text = result['mobile_number'];
                    _additemdes.text = _quantity[0];
                    _partytrc.text = result['tnc'];
  ...
  void _AllProductItemList(context) {
  ...
  Widget build(BuildContext context) {
    void _addlink() {
      _childrenlink = List.from(_childrenlink)
        ..add(Container(
          child: Row(
            children: <Widget>[
              Expanded(
